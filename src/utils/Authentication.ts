...
class Authentication {
    ...
    public static async passwordCompare(text:string, encryptText:string): Promise<boolean> {
        return await bcrypt.compare(text, encryptText);
    }

    public static generateToken(id: number, email:string, name: string, username: string): string {
        const secretKey: string = process.env.JWT_SECRET_KEY || "my-secret";
        const payload: Payload = {userId: id, email:email, name:name, username:username};
